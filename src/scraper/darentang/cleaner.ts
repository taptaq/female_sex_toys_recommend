...
const mapGender = (raw: string, format: 'lowercase' | 'capitalized' = 'lowercase'): string => {
  ...
  else if (val.includes('female') || val.includes('女性') || val.includes('她') || val.includes('妇')) result = 'female';
  else if (val.includes('male') || val.includes('男性') || val.includes('他') || val.includes('男士')) result = 'male';
...
const mergeSpecsWithDefaults = (defaults: any, parsed: any) => ({
  ...defaults,
  ...(parsed || {}),
  function_tags: Array.isArray(parsed?.function_tags) ? parsed.function_tags : defaults.function_tags,
...
`;
    ...
    try {
      ...
      const resolvedGender = productKind === 'care'
        ? 'unisex'
        : mapGender(parsedSpecs.gender || item.genderHint || 'female');
