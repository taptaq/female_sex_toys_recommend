...
const CLEANED_PATH = path.resolve(__dirname, '../../data/leitingbaofeng-cleaned-data.json');
...
export async function runCleaner() {
  console.log('\n======================================================');
  console.log('--- 启动 雷霆暴风 (Leitingbaofeng / LETEN) AI 清洗与入库模块 ---');
  console.log('======================================================');
  ...
  // --- 预搜索 雷霆暴风 在 competitors 表中的 ID ---
  let brandId: string | null = null;
  try {
    const competitor = await prisma.competitors.findFirst({
      where: { name: { contains: '雷霆暴风', mode: 'insensitive' } }
    });
    if (competitor) {
      brandId = competitor.id;
      console.log(`[关联] 已定位 雷霆暴风 竞品 ID: ${brandId}`);
    } else {
        // 如果不存在，尝试创建一个基础记录
        console.log('[创建] 数据库中未发现「雷霆暴风」，正在初始化记录...');
        const newBrand = await prisma.competitors.create({
            data: {
                name: '雷霆暴风',
                description: '雷霆暴风（Leitingbaofeng / LETEN）是中国情趣品牌，覆盖男性、女性与双人互动产品线。',
                is_domestic: true
            }
        });
        brandId = newBrand.id;
        console.log(`[创建] 已创建 雷霆暴风 竞品记录 (ID: ${brandId})`);
    }
  } catch (err) {
  ...
  for (const item of bufferData) {
    ...
    const prompt = productKind === 'care'
      ? `
你是一个个人护理耗材商品目录数据清洗助手。现有抓取至「雷霆暴风 (Leitingbaofeng / LETEN)」天猫店的安全套/润滑液/护理用品类商品描述：
...
`
      : productKind === 'pad'
      ? `
你是一个家居床品防护垫商品目录数据清洗助手。现有抓取至「雷霆暴风 (Leitingbaofeng / LETEN)」天猫店的床事垫/防水垫/护理垫类商品描述：
...
`
      : productKind === 'apparel'
      ? `
你是一个服装商品目录数据清洗助手。现有抓取至「雷霆暴风 (Leitingbaofeng / LETEN)」天猫店的服饰类商品描述：
...
`
      : `
你是一个专注处理个人护理器具参数的数据拆解机器人。现有抓取至「雷霆暴风 (Leitingbaofeng / LETEN)」天猫店的纯文本描述：
...
注意：雷霆暴风产品线可能同时覆盖男性、女性与双人互动，请优先依赖文本显式信号。
字段要求：
{
  "max_db": 50,
  "waterproof": 7,
