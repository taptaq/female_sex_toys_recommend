...
export const ORIGIN = 'https://lovecrave.com';
export const LIST_URL = `${ORIGIN}/collections/all?filter.v.price.gte=&filter.v.price.lte=&sort_by=best-selling`;
...
export const MAX_ITEMS = Number(process.env.LOVECRAVE_OFFICIAL_MAX_ITEMS || '200');
...
export type LovecraveProductDetail = {
  title: string;
  subtitle: string;
  metaTitle: string;
...
export type LovecraveReviewBufferItem = LovecraveListItem &
  LovecraveProductDetail & {
    isReviewed: false;
  };
...
function normalizeTags(tags: ShopifyProduct['tags']): string[] {
  if (Array.isArray(tags)) {
    return uniqueStrings(tags.map((value) => String(value || '')));
  }
...
export function shouldKeepLovecraveCandidate(input: Record<string, unknown>): boolean {
  const nameText = [String(input.name || ''), String(input.subtitle || '')].join(' ').toLowerCase();
  const rawDescription = String(input.rawDescription || '').toLowerCase();
  const haystack = buildCandidateText(input);
  if (!haystack) return false;

  const strongToySignals = [
    'vibrator',
    'vibe',
    'bullet',
    'stimulator',
  ...
  const strongToyMatch = strongToySignals.some((signal) => haystack.includes(signal));
  const explicitToyTitleMatch = strongToySignals.some((signal) => nameText.includes(signal));

  const hardBlockedPatterns = [/\bgift\s*card\b/i, /\bcharger\b/i, /\bcable\b/i, /\bcharging\s*cable\b/i];
  if (hardBlockedPatterns.some((pattern) => pattern.test(haystack))) {
    return false;
  }

  const pureAccessorySignals = [
    /\bcuban\s+chain\b/i,
    /\bchain\b/i,
    /\bcap\b/i,
    /\battachment\b/i,
  ...
  const looksLikePureAccessory = pureAccessorySignals.some((pattern) => pattern.test(haystack));
  const titleLooksAccessoryOnly =
    (/\bchain\b/i.test(nameText) || /\bcap\b/i.test(nameText) || /\battachment\b/i.test(nameText)) &&
    !explicitToyTitleMatch;
  const compatibilityOnlyAccessory =
    /\bcompatible with\b/i.test(rawDescription) ||
    /\bonly necklace included\b/i.test(rawDescription) ||
    /\breplace(?: it| the ring band)?\b/i.test(rawDescription);

  if (titleLooksAccessoryOnly) {
    return false;
  }

  if (looksLikePureAccessory && (!strongToyMatch || compatibilityOnlyAccessory)) {
    return false;
  }

  return strongToyMatch;
}
...
function resolveListItemSubtitle(block: string, name: string): string {
  const candidates = [
    findFirstCapture(block, [
      ...
      /<div[^>]*class="[^"]*card__content[^"]*"[^>]*>([\s\S]*?)<\/div>/i,
    ]),
  ].filter(Boolean);
...
export function extractListItemsFromHtml(html: string): LovecraveListItem[] {
  const gridHtml = extractElementById(html, 'product-grid') || html;
  const blocks = extractGridItemBlocks(gridHtml);
  const candidates = blocks.length > 0 ? blocks : extractAnchorBlocks(gridHtml);
  const items: LovecraveListItem[] = [];
  ...
  for (const [index, block] of candidates.entries()) {
    ...
    if (sourceUrl && shouldKeepLovecraveCandidate(item)) {
      items.push(item);
    }
  }
...
export function extractListItemsFromShopifyJson(
  ...
  return products
    .map((product, index) => {
      ...
      const subtitle = normalizeWhitespace(String(product.product_type || '')) || normalizeWhitespace(String(product.body_html || ''));
      const categoryHints = uniqueStrings([String(product.product_type || ''), ...normalizeTags(product.tags)]);
    ...
    .filter((item) => item.sourceUrl && shouldKeepLovecraveCandidate(item));
}

export function mergeUniqueListItems(items: LovecraveListItem[]): LovecraveListItem[] {
  const byUrl = new Map<string, LovecraveListItem>();
...
export function extractRelevantDetailTextFromHtml(html: string): string {
  ...
  return uniqueStrings([...hintedSections, ...genericText], 120).join('\n');
}

export function buildLovecraveRawDescription(parts: Array<string | null | undefined>): string {
  return uniqueStrings(parts, 160).join('\n');
}

export function extractDetailFromHtml(html: string): LovecraveProductDetail {
  const title =
  ...
  const rawDescription = buildLovecraveRawDescription([title, subtitle, metaDescription, extractRelevantDetailTextFromHtml(html)]);
...
export function extractDetailFromShopifyProduct(product: ShopifyProduct): LovecraveProductDetail {
  const { priceUsd, originalPriceUsd } = resolveShopifyProductPrice(product);
  ...
  const rawDescription = buildLovecraveRawDescription([
...
function normalizeDetail(detail: LovecraveProductDetail): LovecraveProductDetail {
  return {
    ...detail,
...
function buildReviewBufferItem(item: LovecraveListItem, detail: LovecraveProductDetail): LovecraveReviewBufferItem | null {
  if (
...
export async function crawlDetailItems(
  items: LovecraveListItem[],
  fetchDetail: (item: LovecraveListItem) => Promise<LovecraveProductDetail>,
): Promise<LovecraveReviewBufferItem[]> {
  const result: LovecraveReviewBufferItem[] = [];
  ...
  for (const item of items) {
    try {
      ...
      console.warn(`[lovecrave-official] 详情抓取失败，跳过 ${item.sourceUrl}:`, error);
    }
  }
...
export async function crawlListingPages(input: CrawlListingPagesInput): Promise<LovecraveListItem[]> {
  ...
  try {
    ...
    if (html && !isShopifyErrorPage(html)) {
      const htmlItems = mergeUniqueListItems(extractListItemsFromHtml(html));
      if (htmlItems.length > 0) {
        return htmlItems.slice(0, maxItems);
      }
    }
  } catch (error) {
    console.warn('[lovecrave-official] 列表 HTML 抓取失败，回退 Shopify JSON。', error);
  }

  const jsonItems: LovecraveListItem[] = [];
  ...
  return mergeUniqueListItems(jsonItems).slice(0, maxItems);
}
...
function mergeDetailSources(primary: LovecraveProductDetail, secondary: LovecraveProductDetail): LovecraveProductDetail {
  return {
    ...
    rawDescription: buildLovecraveRawDescription([primary.rawDescription, secondary.rawDescription]),
  };
}

async function fetchLovecraveDetail(
  item: LovecraveListItem,
  preloadedProduct: ShopifyProduct | null = null,
): Promise<LovecraveProductDetail> {
  ...
  try {
    ...
    console.warn(`[lovecrave-official] 商品页 HTML 解析失败，回退 JSON: ${item.sourceUrl}`, error);
  }
...
export function writeReviewBuffer(rows: LovecraveReviewBufferItem[]) {
  ensureDir(BUFFER_PATH);
  fs.writeFileSync(BUFFER_PATH, JSON.stringify(rows, null, 2));
}

export async function runCrawler(): Promise<LovecraveReviewBufferItem[]> {
  ...
  const reviewBuffer = await crawlDetailItems(listItems, async (item) => {
    ...
    if (preloadedProduct) {
      return extractDetailFromShopifyProduct(preloadedProduct);
    }

    return fetchLovecraveDetail(item, null);
  });
