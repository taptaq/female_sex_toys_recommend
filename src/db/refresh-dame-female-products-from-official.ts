...
export const DAME_BRAND_NAME = "Dame";
export const DAME_FEMALE_LIST_URL =
  process.env.DAME_OFFICIAL_LIST_URL || "https://dame.com/collections/all";
export const DAME_REVIEW_BUFFER_PATH = "src/data/dame-all-products-female-review-buffer.json";
export const DAME_USD_TO_CNY_RATE = Number(process.env.DAME_USD_CNY_RATE || "7.1200");
const DAME_REFRESH_BATCH_SIZE = Number(process.env.DAME_REFRESH_BATCH_SIZE || "30");
const DAME_OFFICIAL_MAX_ITEMS = Number(process.env.DAME_OFFICIAL_MAX_ITEMS || "200");
const DAME_ORIGIN = "https://dame.com";
...
const DAME_COMPETITOR_CONFIG: CompetitorRegistryConfig = {
  canonicalName: DAME_BRAND_NAME,
  matchNames: ["dame", "dame products"],
  domain: "dame.com",
  country: "美国",
  foundedDate: "2014",
  description: "Dame 是美国女性向 sexual wellness 品牌，覆盖吸吮、震动、穿戴、G 点/插入、双刺激和亲密感官玩法。",
  focus: "Female",
  philosophy: [
    "以女性本位和研究驱动的方式设计亲密产品。",
    "强调身体友好、日常可用和更少羞耻感的愉悦体验。",
    "把玩具、护理和亲密教育放在同一个 sexual wellness 语境中，但玩具产品仍保持明确功能分层。",
  ],
  majorUserGroupProfile:
    "【核心人口】25-45 岁女性用户、新手与有经验的身体探索者，以及伴侣共玩人群。\n【心理特征】重视品牌可信度、材质安全、人体工学和不过度羞耻化的表达。\n【核心痛点】希望在护理、教育和玩具混合货架中快速区分真实刺激类型，避免把配件或护理品误当核心玩具。",
  isDomestic: false,
};
...
function normalizeProductUrl(href: unknown, baseUrl = DAME_FEMALE_LIST_URL) {
...
function normalizeImageUrl(src: unknown) {
  ...
  if (trimmed.startsWith("/")) return `${DAME_ORIGIN}${trimmed}`;
  return trimmed;
}
...
function buildTrustedSource(row: Partial<DameSourceRow>) {
...
function isObviousNonToy(row: Partial<DameSourceRow>) {
  const source = buildTrustedSource(row);
  return /warehouse sale|gift card|shipping protection|routeins|sti\b|test kit|screening|gonorrhea|chlamydia|syphilis|lube|lubricant|cleaner|body wipes|wipes|serum|gummies|supplement|oil|candle|condom|blanket|pillo|pillow|workbook|method book|\bbook\b|socks|\bhat\b|bucket hat|\bstash\b|\bbag\b|pouch|\bgrip\b|charging base|charging cable|charger|spare|replacement|outer cover|component|accessories|accessory|donation|mystery item/.test(source);
}

function isObviousMaleOnly(row: Partial<DameSourceRow>) {
  const source = buildTrustedSource(row);
  return /stroker|masturbator|sleeve|penis|\bmale\b|\bfor him\b|prostate|cock ring|cockring|vibrating cockrings|男用|男性|阴茎|陰茎/.test(source);
}

export function shouldKeepDameFemaleSourceRow(row: Partial<DameSourceRow>) {
  ...
  const source = buildTrustedSource(row);
  return /sex toy|toy\b|vibrator|vibe\b|suction|air pulse|clitoral|g-?spot|dildo|yoni egg|yoni eggs|butt plug|plug\b|anal|finger vibrator|panty vibrator|remote-controlled|remote controlled|wearable|rabbit|wand|bullet|bondage|rope|blindfold|tickler|feather|emojibator|chakrubs|crystal dildo|pleasure antlers|pleasure ears|pleasure nose|pleasure trunk/.test(source);
}

function inferGender(row: Partial<DameSourceRow>): Gender {
  const source = buildTrustedSource(row);
  if (/couples|partner|panty|remote|anal|plug|bondage|rope|blindfold|tickler|feather|yoni egg/.test(source)) return "unisex";
  return "female";
}

function resolveTypePatch(row: Partial<DameSourceRow>) {
  const source = buildTrustedSource(row);
  const tags = Array.isArray(row.categoryHints) ? row.categoryHints : [];
  const physicalForm =
    /rabbit|dual stimulation|dual action|dual vibe|cora|kitti|honey bunny|royale bunny|g-?spot.{0,80}clitoral|clitoral.{0,80}g-?spot/.test(source)
      ? "composite"
      : /dildo|yoni egg|yoni eggs|plug|anal|internal|insertable|g-?spot|arc|amethyst|anahata|quartz|jade|obsidian|viper|chakrubs|cecii|kalii|njoy pure wand|pure wand/.test(source)
        ? "internal"
        : "external";
  ...
  if (/bondage rope|blindfold|tickler|feather|funfetti|unicorn rainbow|sleep eye mask|sensory/.test(source)) {
    return {
      typeCode: "bdsm",
      subtypeCode: /rope|bondage/.test(source) ? "bondage_restraint" : "sensory_play",
      maxDb: 0,
      waterproof: 1,
    };
  }
  if (/panty vibrator|lay panty|remote-controlled|remote controlled|remote\b|wearable|eva wearable|romp cello|temptasia heartbeat/.test(source)) {
    return { typeCode: "wearable_remote", subtypeCode: "panty_wearable", maxDb: 40, waterproof: 7 };
  }
  if (/rabbit|dual stimulation|dual action|dual vibe|cora|kitti|honey bunny|royale bunny|double stimulation|双重刺激/.test(source)) {
    return { typeCode: "dual_stimulation", subtypeCode: /suction|吸吮|吮吸/.test(source) ? "suction_dual" : "rabbit_dual", maxDb: 40, waterproof: 7 };
  }
  if (/aer|suction|air pulse|sucking|rosales|strawberry emojibator/.test(source)) {
    return { typeCode: "suction", subtypeCode: "suction_pure", maxDb: 40, waterproof: 7 };
  }
  if (/dildo|yoni egg|yoni eggs|butt plug|plug\b|anal|insertable|g-?spot|arc|amethyst|anahata|quartz|jade|obsidian|viper|chakrubs|cecii|kalii|njoy pure wand|pure wand|romp piccolo/.test(source)) {
    const powered = /vibrator|vibe|vibrating|moving beads|beads|rotating|thumping|rechargeable/.test(source);
    return { typeCode: "insertable", subtypeCode: powered ? "insertable_vibe" : "gspot_insertable", maxDb: powered ? 40 : 0, waterproof: 7 };
  }
  if (/wand|magic wand|candle warming wand|mini wand|min mini wand/.test(source)) {
    return { typeCode: "external_vibe", subtypeCode: "wand_massager", maxDb: 40, waterproof: 7 };
  }
  if (/bullet|finger vibrator|finger and clit|fin|zee|pom|dip|banana emojibator|cherry emojibator|chili pepper|eggplant|pickle|tiny wand|iroha petit|pebble|clitoral vibrator|vibrator|vibe\b|emojibator|pleasure antlers|pleasure ears|pleasure nose|pleasure trunk/.test(source)) {
...
function normalizePhysicalForm(typeCode: string, source: string): "external" | "internal" | "composite" {
  if (typeCode === "dual_stimulation" || /rabbit|dual stimulation|dual action|dual vibe/.test(source)) return "composite";
  if (typeCode === "insertable" || /dildo|yoni egg|yoni eggs|plug\b|anal|internal|insertable|g-?spot|chakrubs|crystal|arc/.test(source)) {
    return "internal";
  }
  return "external";
}

function inferMaterial(row: Partial<DameSourceRow>) {
  const source = buildTrustedSource(row);
  if (/amethyst|quartz|jade|obsidian|rhodonite|crystal|chakrubs/.test(source)) return "天然水晶/石材";
  if (/glass|borosilicate/.test(source)) return "硼硅玻璃";
  if (/silicone|硅胶/.test(source)) return "亲肤硅胶";
  if (/rope/.test(source)) return "绳索纤维";
  if (/blindfold|eye mask/.test(source)) return "柔软织物";
  return "亲肤安全材质";
}

function normalizeMotorType(source: string, typeCode: string): "gentle" | "strong" {
  if (typeCode === "bdsm" || (/crystal|stone|glass|dildo|yoni egg|plug/.test(source) && !/vibrating|vibrator|vibe/.test(source))) return "gentle";
  return /wand|magic wand|powerful|deep|rumbly|strong|intense|high-intensity|thumping|rotating/.test(source) ? "strong" : "gentle";
}

function normalizeAppearance(source: string) {
  return /zee|fin|dip|eva|pom|aer|min|bullet|finger|travel|portable|small|discreet|beginner|quiet|on the go|wearable|panty/.test(source)
    ? "high_disguise"
    : "normal";
}

function inferTagsFromText(row: Partial<DameSourceRow>) {
  ...
  push("吮吸刺激", /suction|air pulse|sucking|aer|rosales/);
  push("阴蒂刺激", /clitoral|external|finger and clit|finger vibrator|bullet|zee|fin|pom|dip|wand/);
  push("G点刺激", /g-?spot|arc|g点/);
  push("入体探索", /internal|insertable|dildo|yoni egg|yoni eggs|plug|chakrubs|crystal|按摩棒/);
  push("后庭探索", /anal|butt plug|plug\b/);
  push("穿戴遥控", /panty|remote|wearable|eva|lay|romp cello|temptasia/);
  push("BDSM", /bondage|rope|blindfold|tickler|feather/);
  push("情侣共玩", /couples|partner|remote|wearable/);
  ...
  push("可充电", /rechargeable|usb|charging|可充电|充电/);
  push("震动刺激", /vibrator|vibe|vibrating|wand|bullet|thumping|rotating|震动|振动/);
...
function normalizeRmbPrice(row: Partial<DameSourceRow>) {
  ...
  return normalizePositivePrice((Number.isFinite(priceUsd) && priceUsd > 0 ? priceUsd : 1) * DAME_USD_TO_CNY_RATE);
}

function buildRecommendationFeaturesForPatch(patch: Omit<DameFemaleRefreshPatch, "recommendationFeatures">) {
    ...
    : [
        {
          ...
          text: "Dame 官方资料显示该产品面向女性友好身体探索或伴侣亲密玩法",
...
function buildRawDescription(product: ShopifyProduct, row: Omit<DameSourceRow, "rawDescription">) {
  return [
    "[基础信息]",
    `商品名: ${row.name}`,
    ...
    "",
    "[英文详情]",
    normalizeBlock(product.body_html) || `${row.name} official Dame product.`,
    "",
    `[来源链接] ${row.sourceUrl}`,
  ]
    .filter(Boolean)
...
function sourceRowFromShopifyProduct(product: ShopifyProduct, listPosition: number): DameSourceRow | null {
  const handle = normalizeText(product.handle);
  const sourceUrl = buildProductUrlFromHandle(handle);
  const name = normalizeNonEmpty(product.title, "Dame 未命名商品");
  const { priceUsd, originalPriceUsd } = resolvePriceFromProduct(product);
  const categoryHints = uniqueStrings([product.product_type, ...normalizeTags(product.tags), "Dame All Products"], 80);
  ...
  const rowWithoutDescription = {
    ...
    specs: {
      ...
      price_rmb: normalizePositivePrice((priceUsd ?? 1) * DAME_USD_TO_CNY_RATE),
      fx_rate_usd_cny: DAME_USD_TO_CNY_RATE,
      fx_rate_source: "fixed",
    },
  } satisfies Omit<DameSourceRow, "rawDescription">;
  const row = {
    ...rowWithoutDescription,
    rawDescription: buildRawDescription(product, rowWithoutDescription),
  } satisfies DameSourceRow;
  return shouldKeepDameFemaleSourceRow(row) ? { ...row, genderHint: inferGender(row) } : null;
}

export function extractDameFemaleRowsFromShopifyCatalog(payload: ShopifyCatalogResponse | ShopifyProduct[]) {
  const products = Array.isArray(payload) ? payload : payload.products ?? [];
  return products
    .map((product, index) => sourceRowFromShopifyProduct(product, index + 1))
    .filter((row): row is DameSourceRow => Boolean(row));
}

export function buildDameFemaleRefreshPatch(row: Partial<DameSourceRow>): DameFemaleRefreshPatch {
  const name = normalizeNonEmpty(row.name, "Dame 未命名商品");
  const rawDescription = normalizeNonEmpty(
    row.rawDescription,
    `[基础信息]\n商品名: ${name}\n站内分类提示: Dame All Products\n性别提示: female/unisex\n[英文详情]\nDame official female-friendly product from ${DAME_FEMALE_LIST_URL}.\n[来源链接] ${normalizeProductUrl(row.sourceUrl) || DAME_FEMALE_LIST_URL}`,
  );
  ...
  const patchWithoutFeatures = {
    ...
    brand: DAME_BRAND_NAME,
    material: normalizeNonEmpty(row.specs?.material, inferMaterial({ ...row, rawDescription })),
    link: normalizeNonEmpty(normalizeProductUrl(row.sourceUrl), DAME_FEMALE_LIST_URL),
    ...
    productSpecs: {
      ...
      officialListUrl: DAME_FEMALE_LIST_URL,
      price_source_currency: "USD",
  ...
  } satisfies Omit<DameFemaleRefreshPatch, "recommendationFeatures">;
...
export async function fetchDameOfficialSourceRows() {
  const rows: DameSourceRow[] = [];
  for (let page = 1; rows.length < DAME_OFFICIAL_MAX_ITEMS; page += 1) {
    const url = new URL(`${DAME_FEMALE_LIST_URL.replace(/\/$/, "")}/products.json`);
    url.searchParams.set("limit", "250");
    url.searchParams.set("page", String(page));
    const payload = await fetchJson<ShopifyCatalogResponse>(url.toString());
    const products = Array.isArray(payload.products) ? payload.products : [];
    if (products.length === 0) break;
    rows.push(...extractDameFemaleRowsFromShopifyCatalog(products));
    console.log(`[refresh-dame-female-products-from-official] 已抓取列表页 ${page}，累计 ${rows.length}`);
    if (products.length < 250) break;
  }
  const limited = rows.slice(0, DAME_OFFICIAL_MAX_ITEMS);
  fs.writeFileSync(DAME_REVIEW_BUFFER_PATH, `${JSON.stringify(limited, null, 2)}\n`);
  return limited;
}

async function ensureDameCompetitor(client: PgClientLike) {
  const prismaLike = {
    competitors: {
        ...
        const result = await client.query(
          `
            ...
            WHERE lower(name) = ANY($1::text[])
               ...
               OR lower(coalesce(domain, '')) LIKE '%dame.com%'
            LIMIT 1
          `,
          [
  ...
  return await ensureCompetitorRecord({
    ...
    brandName: DAME_BRAND_NAME,
    overrideConfig: DAME_COMPETITOR_CONFIG,
  });
}

async function ensureProductForPatch(client: PgClientLike, patch: DameFemaleRefreshPatch, competitorId: string | null) {
...
async function upsertFemaleToy(client: PgClientLike, patch: DameFemaleRefreshPatch, competitorId: string | null) {
...
async function backfillIncompleteDameRows(client: PgClientLike, competitorId: string | null) {
  ...
  for (const row of incomplete.rows) {
    const name = normalizeNonEmpty(row.name ?? row.safe_display_name, "Dame 未命名商品");
    const sourceRow: Partial<DameSourceRow> = {
      sourceUrl: normalizeNonEmpty(row.link, `${DAME_FEMALE_LIST_URL}#${row.id}`),
      name,
      safeDisplayName: normalizeNonEmpty(row.safe_display_name, buildSafeDisplayName(name)),
      subtitle: normalizeNonEmpty(row.subtype_code, "Dame official female-friendly product"),
      coverImage: normalizeNonEmpty(row.image_url, placeholderImageForSubtype(row.subtype_code || "")),
      rawDescription: normalizeNonEmpty(
        row.raw_description,
        `[基础信息]\n商品名: ${name}\n站内分类提示: Dame official female-friendly product\n性别提示: ${normalizeText(row.gender) || "female"}\n价格: 历史记录价格\n[英文详情]\nDame 官方女性/伴侣友好产品记录，字段由兜底清洗流程补齐。\n[来源链接] ${normalizeNonEmpty(row.link, DAME_FEMALE_LIST_URL)}`,
      ),
      detailImageUrls: [],
      colors: [],
      skuList: [],
      categoryHints: uniqueStrings([row.type_code, row.subtype_code, row.physical_form, row.link, "Dame official"], 30),
      genderHint: normalizeLower(row.gender) === "unisex" ? "unisex" : "female",
      productType: normalizeText(row.type_code),
      vendor: DAME_BRAND_NAME,
      handle: "",
      listPosition: 0,
      specs: {
    ...
    const patch = buildDameFemaleRefreshPatch(sourceRow);
  ...
  console.log(`[refresh-dame-female-products-from-official] 已兜底修复不完整行 ${incomplete.rows.length} 条`);
  return incomplete.rows.length;
}

export function shouldRunDameFemaleRefreshScript(importMetaUrl: string, argvEntry?: string) {
  return Boolean(argvEntry && importMetaUrl === pathToFileURL(argvEntry).href);
}

async function loadDameRows() {
  const sourceMode = normalizeLower(process.env.DAME_REFRESH_SOURCE || "live");
  if (sourceMode === "buffer") {
    return {
      sourceMode,
      rows: JSON.parse(fs.readFileSync(DAME_REVIEW_BUFFER_PATH, "utf8")) as DameSourceRow[],
    };
  }
  return { sourceMode, rows: await fetchDameOfficialSourceRows() };
}

async function runDameFemaleRefreshAttempt() {
  const { sourceMode, rows } = await loadDameRows();
  const patches = rows.filter(shouldKeepDameFemaleSourceRow).map(buildDameFemaleRefreshPatch);
  ...
  try {
    ...
    const competitorId = await ensureDameCompetitor(client);
    for (let index = 0; index < patches.length; index += DAME_REFRESH_BATCH_SIZE) {
      const batch = patches.slice(index, index + DAME_REFRESH_BATCH_SIZE);
      await client.query("BEGIN");
      try {
        for (const patch of batch) await upsertFemaleToy(client!, patch, competitorId);
        await client.query("COMMIT");
        console.log(`[refresh-dame-female-products-from-official] 已提交 ${Math.min(index + batch.length, patches.length)}/${patches.length}`);
      } catch (error) {
    ...
    await backfillIncompleteDameRows(client, competitorId);
    ...
    console.log(JSON.stringify({ brand: DAME_BRAND_NAME, source: DAME_FEMALE_LIST_URL, sourceMode, inputRows: rows.length, refreshed: patches.length, ...audit.rows[0] }, null, 2));
  } finally {
    client?.release();
...
async function runDameFemaleRefresh() {
  let lastError: unknown;
  for (let attempt = 1; attempt <= 3; attempt += 1) {
    try {
      await runDameFemaleRefreshAttempt();
      return;
    } catch (error) {
      lastError = error;
      ...
      console.warn(`[refresh-dame-female-products-from-official] 遇到瞬断，重试 ${attempt}/3...`, error);
...
if (shouldRunDameFemaleRefreshScript(import.meta.url, process.argv[1])) {
  runDameFemaleRefresh().catch((error) => {
    console.error("[refresh-dame-female-products-from-official] 执行失败:", error);
    process.exitCode = 1;
  });
}
